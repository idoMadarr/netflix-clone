...
app.use('/favorite', favoriteRoutes_1.default);
app.use((err, req, res, next) => {
    throw new Error(`Error ${500}: ${err.message || 'An unknown error occurred'}`);
});
mongoose_1.connect(URI, { useNewUrlParser: true, useUnifiedTopology: true, useCreateIndex: true, useFindAndModify: false })
    .then(() => {
    app.listen(PORT);
